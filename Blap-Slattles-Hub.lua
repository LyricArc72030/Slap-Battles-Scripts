...
CombatSection:NewToggle("test", "ToggleInfo", function()
    local TourVoid = Instance.new("Part", workspace)
    TourVoid.Position = Vector3.new(3415,71,-16)   
    TourVoid.Name = "Block"  
    TourVoid.Size = Vector3.new(2000,7,1500)   
    TourVoid.Anchored = true  
    TourVoid.Transparency = .9
    Time = 0

    local TourVoid2 = Instance.new("Part", workspace)
    TourVoid2.Position = Vector3.new(3415,71,-16)   
    TourVoid2.Name = "Block2"  
    TourVoid2.Size = Vector3.new(1,1,1)   
    TourVoid2.Anchored = true  
    TourVoid2.Transparency = .9
    Time = 0
    else
            for i,v in pairs(game.Workspace:GetDescendants()) do    
                if v.Name == "Block"  or v.Name == "Block2" then
                    if v.CanCollide == false then
                        v.CanCollide = true        
                        v.Transparency = 1
                    end
